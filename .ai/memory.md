...
| 2026-06-21 | 波形算法归一化重构：新建 `core/ai/algorithms/` 包统一所有波形算法（事件检测/分段/降采样），算法逻辑保持不变。`base.py` 定契约（`Signal` 输入；`Event`/`Segment`/`EventResult`/`SegmentResult` 输出，`to_dict(factor)` 出 JSON；`WaveformAlgorithm` 抽象基类 name/kind/params_cls+run()；工具 `infer_dt`/`trapezoid`/`statistics`）；`registry.py` 注册表 `register/get(name)/available(kind)`（按名互换算法做参数对照）；`swed.py`（SWED 从 tests/_swed_tmp.py 移植，睡眠门限 floor=sleep_floor 0.4mA + 相对回落 theta_end=0.50，实测 test.csv 峰 33.27mA、1.086~5.0s 持续平台保留为单个 level 事件=预期）；`stalta.py`（STA-LTA+MAD 闸门）；`pelt.py`（PELT 变点+classify_segment+segment_features）；`downsample.py`（lttb/adaptive）。`waveform_provider.py` 原算法函数全部迁出，`detect_events_stalta/detect_segments_pelt/segment_features` 改薄适配委托新包（旧 dict 输出/键名不变，向后兼容），删本地 numpy import（已下沉 base.py），`lttb_downsample/adaptive_downsample` 改 import | core/ai/algorithms 同样禁 Qt（在 core 下）；算法内部不做单位换算，量纲交 to_dict(factor)/调用方 _pick_scale；新增算法只需 @register 一个 WaveformAlgorithm 子类即并存可调试；env 无 ruff 用 py_compile + IDE 诊断 + test.csv 实跑校验（swed 9 事件/stalta 6 区间/segment_features 键名一致） |
