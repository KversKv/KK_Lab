...
class ExecutionContext:
    ...
    def acquire_leases(self, owner: str = "orchestrator") -> None:
    ...
    def set_run_snapshot(self, nodes: List[Any], sequence_hash: str = "") -> None:
        from core.orchestrator.snapshot import canonical_sequence_data, build_sequence_hash
    ...
    def execute_children(self, children: List[Any]) -> None:
        if self._execute_children_callback is not None:
            self._execute_children_callback(children)
            return
        from core.orchestrator.runtime import execute_children
        execute_children(children, self)

    def set_prompt_handler(self, handler: Callable[[str, float], Optional[str]]) -> None:
        self._prompt_handler = handler

    def request_user_prompt(self, message: str, timeout_s: float = 300.0) -> Optional[str]:
