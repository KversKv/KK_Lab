...
from core.orchestrator.nodes.base import BaseNode, register_node
from core.orchestrator.result_store import build_default_result_path
...
class ExportResult(BaseNode):
    ...
    def execute(self, context: Any) -> None:
        ...
        if not output_dir:
            ...
            if not filename or filename == "orchestrator_result":
                ...
                output_dir = os.path.join(project_root, "Results", "orchestrator", timestamp)
                filepath = os.path.join(output_dir, f"{filename}.{extension}")
        else:
            filepath = os.path.join(output_dir, f"{filename or 'orchestrator_result'}.{extension}")
