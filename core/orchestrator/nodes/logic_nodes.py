...
class LoopRange(BaseNode):
    ...
    def execute(self, context: Any) -> None:
        ...
        for idx, val in enumerate(values):
            ...
            try:
                context.execute_children(self.children)
            except BreakLoop:
    ...
    def execute(self, context: Any) -> None:
        for child in self.children:
            if isinstance(child, IfBranch) or isinstance(child, ElseIfBranch):
                ...
                if result:
                    context.execute_children(child.children)
                    return
...
class IfThenElse(BaseNode):
    ...
    def execute(self, context: Any) -> None:
        ...
        if result:
            context.execute_children(self.children[:else_idx])
        else:
            context.execute_children(self.children[else_idx:])
...
class PromptUser(BaseNode):
    ...
    def execute(self, context: Any) -> None:
        ...
        if wait_confirm:
            response = context.request_user_prompt(message, timeout_s=300.0)
            if result_var:
                context.set_variable(result_var, response or "confirmed")
