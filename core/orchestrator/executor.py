...
from core.orchestrator.context import (
    ExecutionContext, StopExecution, TestResultException,
)
from core.orchestrator.runtime import execute_children, execute_node
...
class OrchestratorExecutor(QObject):
    """自定义测试执行器（在工作线程中运行）"""
    ...
    def run(self) -> None:
        ...
        if not self._context:
            ...
            return
        self._context.set_child_executor(lambda children: execute_children(children, self._context))
    ...
    def _estimate_total_steps(nodes: List[BaseNode], context: ExecutionContext) -> int:
        ...
        for node in nodes:
            total += 1
            if node.children:
                iterations = OrchestratorExecutor._get_loop_iterations(node, context)
                child_steps = OrchestratorExecutor._estimate_total_steps(node.children, context)
...
class ExecutorThread(QObject):
    ...
    def __init__(self, parent: Optional[QObject] = None) -> None:
        ...
        self._executor: Optional[OrchestratorExecutor] = None

    @property
    def executor(self) -> Optional[OrchestratorExecutor]:
        return self._executor

    def start(self, nodes: List[BaseNode], context: ExecutionContext) -> OrchestratorExecutor:
        """启动执行"""
        self._force_stop()
