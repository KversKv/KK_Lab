...
- `pmu_test/dcdc/dcdc_analysis.generate_current_points` Linear/Log 模式必含 `end_current_a` 终点（2026-09，步进/网格不足时自动补点，相对 1e-9 近重去重）；Custom 模式取 `custom_points`（A 列表），abs/剔 0/去重/升序，空表回退 `[start_a, end_a]`。
