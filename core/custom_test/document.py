...
from core.custom_test.serialization import load_sequence_data, save_sequence_data
...
class CustomTestDocument:
    ...
    @classmethod
    def from_data(cls, data: Any, *, source_path: Optional[str] = None) -> "CustomTestDocument":
...
def load_document_file(file_path: str) -> CustomTestDocument:
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return CustomTestDocument.from_data(data, source_path=file_path)


def save_document_file(file_path: str, document: CustomTestDocument) -> str:
