...
15. **示波器操作等待规范（16×时基 + 两阶段时基）**：改时基 / 改 Vertical Scale / 获取测量值（DISPlay 查询）任一操作都会触发 DSOX 重新捕获全屏，其后必须等待 `16×时基`（≥1s 下限）才能取到正确值/稳定帧（500ms/div 即 8s）。大时基下逐轮等待过慢，须用**两阶段时基**：测试项每组开始先将时基设为预览值（`周期/10×1.1`，如 100Hz→1.1ms、1000Hz→110us，整屏约 1.1 周期），在预览时基下完成量程搜索（autoscale）等参数调整（每次等待按 16×预览时基计，极短），参数调好后切最终时基（改前须先 `run()` 恢复采集）做正式测量与截图，等待按 16×最终时基计。
---
