...
class PMUOutputVoltageUI(QWidget):
    ...
    def _create_layout(self):
        ...
        # 测试配置区域
        config_group = QGroupBox("Output Voltage测试配置")
        config_layout = QGridLayout()
        config_layout.setSpacing(15)
        config_layout.setContentsMargins(20, 15, 20, 15)
        
        # 输出通道选择
        config_layout.addWidget(QLabel("输出通道:"), 0, 0)
        self.output_channel_combo = QComboBox()
        self.output_channel_combo.addItems(["OUT1", "OUT2", "OUT3", "OUT4"])
        config_layout.addWidget(self.output_channel_combo, 0, 1)
        
        # 设定电压
        config_layout.addWidget(QLabel("设定电压 (V):"), 0, 2)
        self.set_voltage_spin = QDoubleSpinBox()
        self.set_voltage_spin.setRange(0.0, 20.0)
        self.set_voltage_spin.setSingleStep(0.01)
        self.set_voltage_spin.setValue(3.3)
        config_layout.addWidget(self.set_voltage_spin, 0, 3)
        
        # 负载电流范围
        config_layout.addWidget(QLabel("负载电流范围 (mA):"), 1, 0)
        self.load_current_min_spin = QDoubleSpinBox()
        self.load_current_min_spin.setRange(0.0, 5000.0)
        self.load_current_min_spin.setSingleStep(10.0)
        self.load_current_min_spin.setValue(0.0)
        config_layout.addWidget(self.load_current_min_spin, 1, 1)
        ...
        self.load_current_max_spin.setSingleStep(10.0)
        self.load_current_max_spin.setValue(500.0)
        config_layout.addWidget(self.load_current_max_spin, 1, 3)
        
        # 电流步进
        config_layout.addWidget(QLabel("电流步进 (mA):"), 2, 0)
        self.current_step_spin = QDoubleSpinBox()
        self.current_step_spin.setRange(1.0, 100.0)
        self.current_step_spin.setSingleStep(1.0)
        self.current_step_spin.setValue(50.0)
        config_layout.addWidget(self.current_step_spin, 2, 1)
        
        # 测量次数
        config_layout.addWidget(QLabel("测量次数:"), 2, 2)
        self.measure_count_spin = QSpinBox()
        self.measure_count_spin.setRange(1, 100)
        self.measure_count_spin.setValue(5)
        config_layout.addWidget(self.measure_count_spin, 2, 3)
        ...
        # 使能过压保护
        self.ovp_checkbox = QCheckBox("使能过压保护")
        self.ovp_checkbox.setChecked(True)
        config_layout.addWidget(self.ovp_checkbox, 4, 0, 1, 2)
        
        # 过压保护值
        config_layout.addWidget(QLabel("过压保护 (V):"), 4, 2)
        self.ovp_spin = QDoubleSpinBox()
        self.ovp_spin.setRange(0.0, 25.0)
        self.ovp_spin.setSingleStep(0.1)
        self.ovp_spin.setValue(4.0)
        config_layout.addWidget(self.ovp_spin, 4, 3)
        ...
        chart_label = QLabel("输出电压负载特性图表区域")
        ...
        # 平均输出电压
        self.avg_voltage_label = QLabel("平均输出电压: ---")
        self.avg_voltage_label.setStyleSheet("font-size: 14px; font-weight: bold;")
        data_layout.addWidget(self.avg_voltage_label, 0, 0)
        
        # 电压误差
        self.voltage_error_label = QLabel("电压误差: ---")
        self.voltage_error_label.setStyleSheet("font-size: 14px; font-weight: bold;")
        data_layout.addWidget(self.voltage_error_label, 0, 1)
        
        # 电压纹波
        self.ripple_label = QLabel("电压纹波: ---")
        self.ripple_label.setStyleSheet("font-size: 14px; font-weight: bold;")
        data_layout.addWidget(self.ripple_label, 1, 0)
        
        # 负载调整率
        self.load_regulation_label = QLabel("负载调整率: ---")
        self.load_regulation_label.setStyleSheet("font-size: 14px; font-weight: bold;")
        data_layout.addWidget(self.load_regulation_label, 1, 1)
    ...
    def set_test_running(self, running):
        ...
        # 禁用/启用配置控件
        widgets = [
            self.output_channel_combo,
            self.set_voltage_spin,
            self.load_current_min_spin,
            self.load_current_max_spin,
            self.current_step_spin,
    ...
    def update_test_result(self, result):
        ...
        if 'avg_voltage' in result:
            self.avg_voltage_label.setText(f"平均输出电压: {result['avg_voltage']:.4f} V")
        if 'voltage_error' in result:
            self.voltage_error_label.setText(f"电压误差: {result['voltage_error']:.4f}%")
        if 'ripple' in result:
            self.ripple_label.setText(f"电压纹波: {result['ripple']:.4f} mV")
        if 'load_regulation' in result:
            self.load_regulation_label.setText(f"负载调整率: {result['load_regulation']:.4f}%")
    
    def clear_results(self):
        """清空测试结果"""
        self.avg_voltage_label.setText("平均输出电压: ---")
        self.voltage_error_label.setText("电压误差: ---")
        self.ripple_label.setText("电压纹波: ---")
        self.load_regulation_label.setText("负载调整率: ---")
