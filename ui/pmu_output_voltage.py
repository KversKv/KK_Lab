    ...
    def run(self):
        try:
            ...
            while code <= max_code:
                ...
                if len(voltages) >= 1:
                    result["min_voltage"] = min(voltages)
                    result["max_voltage"] = max(voltages)
                if len(voltages) >= 2:
                    avg_step = (voltages[-1] - voltages[0]) / (len(voltages) - 1) * 1000.0
                    result["step_voltage"] = avg_step

                    full_scale = voltages[-1] - voltages[0]
                    if abs(full_scale) > 1e-9:
                        n = len(voltages)
                        ideal_step = full_scale / (n - 1)
                        max_dev = 0.0
                        for j in range(n):
                            ideal_v = voltages[0] + ideal_step * j
                            dev = abs(voltages[j] - ideal_v)
                            if dev > max_dev:
                                max_dev = dev
...
class PMUOutputVoltageUI(QWidget):
    ...
    def update_test_result(self, result):
        ...
        if 'min_voltage' in result:
            self.min_voltage_card["value"].setText(f"{result['min_voltage']:.4f} V")
        if 'max_voltage' in result:
            self.max_voltage_card["value"].setText(f"{result['max_voltage']:.4f} V")
