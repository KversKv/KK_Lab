...
class GPADCTestUI(QWidget):
    ...
    def _on_n6705c_search_done(self, n6705c_devices):
        ...
        if n6705c_devices:
            ...
            if default_device not in n6705c_devices:
        ...
        self.n6705c_connect_btn.setEnabled(True)

    def _on_n6705c_search_error(self, err):
        print(f"Search N6705C error: {err}")
    ...
    def _connect_n6705c(self):
        ...
        try:
            ...
            print(f"Connect N6705C error: {e}")
    ...
    def _disconnect_n6705c(self):
        ...
        try:
            ...
            print(f"Disconnect N6705C error: {e}")
            self._set_status_label(self.n6705c_status, f"Error: {e}", "err")
            self._set_btn_connected(self.n6705c_connect_btn)
    ...
    def _on_vt6002_search_error(self, err):
        print(f"Search VT6002 error: {err}")
    ...
    def _connect_vt6002(self):
        ...
        try:
            ...
            print(f"Connect VT6002 error: {e}")
    ...
    def _disconnect_vt6002(self):
        ...
        try:
            ...
            print(f"Disconnect VT6002 error: {e}")
            self._set_status_label(self.vt6002_status, f"Error: {e}", "err")
            self._set_btn_connected(self.vt6002_connect_btn)
    ...
    def _search_dut_ports(self):
        ...
        worker.error.connect(lambda err: print(f"Search DUT ports error: {err}"))
        worker.finished.connect(thread.quit)
        worker.error.connect(thread.quit)
        thread.finished.connect(worker.deleteLater)
        thread.finished.connect(thread.deleteLater)
    ...
    def _start_test(self):
        ...
        print(f"Starting GPADC test... mode={self.current_test_item}")
        ...
        thread.finished.connect(worker.deleteLater)
        thread.finished.connect(thread.deleteLater)
        thread.finished.connect(self._stop_test)
    ...
    def _run_1000cnt_test(self, device_addr, reg_addr, stop_check=None):
        print(f"Starting 1000CNT TEST with I2C address: 0x{device_addr:x} Register: 0x{reg_addr:x}")
    ...
    def _run_high_low_temp_test(self, device_addr, reg_addr, temp_min, temp_max,
        ...
        print("RUN TEST_HIGH_LOW_TEMP TEST")
    ...
    def _run_temp_consistency_test(self, device_addr, reg_addr, temp_min, temp_max,
        ...
        print("RUN TEST_TEMP_CONSISTENCY TEST")
    ...
    def _on_test_done(self, payload):
        ...
        if kind == '1000cnt':
            ...
            if result is not None:
                ...
                self._plot_voltage_adc_curve(temp_data, mean_cali, adc_min_cali, adc_max_cali)
                self.update_test_result(params)
                self._export_data = {
                    'params': params,
                    'raw': result,
    ...
    def export_result(self):
        ...
        if self._export_data is None:
            print("No test result to export.")
            return
        ...
        if self._chart_image_bytes is not None:
            try:
                ...
                print(f"Embed chart image failed: {ex}")
    ...
    def gpadc_force_voltage_test(
        ...
        print(f"Running FORCE VOLTAGE TEST with I2C address: 0x{device_addr:x}, Register: 0x{reg_addr:x}")
        ...
        if DEBUG_FLAG:
            ...
            if vol_source is None or not self.is_n6705c_connected:
                print("Error: N6705C not connected")
        ...
        while current_voltage <= voltage_max + voltage_step * 0.001:
            if stop_check and stop_check():
                print("Force voltage test stopped by user.")
                break
        ...
        print("\n===== FORCE VOLTAGE TEST 结果 =====")
        print("Voltage, RawMean, RawMin, RawMax")
        for i in range(len(voltage_data)):
            print(f"{voltage_data[i]:.4f}, {adc_mean[i]:.3f}, {adc_min[i]}, {adc_max[i]}")
    ...
    def _calibration_data(self, result):
        ...
        print(f"calibration points: v_low={v_low:.4f}, m_low={m_low:.3f}, v_high={v_high:.4f}, m_high={m_high:.3f}")
        k = (m_high - m_low) / (v_high - v_low)
        b = m_low - k * v_low

        print(f"Calibration: k={k:.6f} (LSB/V), b={b:.6f} (LSB)")

        mean_cali    = [(adc - b) / k for adc in adc_mean]
        adc_min_cali = [(adc - b) / k for adc in adc_min]
        adc_max_cali = [(adc - b) / k for adc in adc_max]

        print('voltage   mean_raw   mean_cali  min_cali   max_cali')
        for i in range(n):
            print(f'{adc_raw_data[i]:.4f}    {adc_mean[i]:.3f}   {mean_cali[i]:.6f}   {adc_min_cali[i]:.6f}   {adc_max_cali[i]:.6f}')
        ...
        try:
            if DEBUG_FLAG:
                ...
                if not hasattr(self, 'vt6002') or not self.is_vt6002_connected:
                    print("Error: VT6002 chamber not connected")
            ...
            while current_temp <= temp_max + 0.001:
                if stop_check and stop_check():
                    print("High/Low temp test stopped by user.")
                    break
                ...
                if DEBUG_FLAG:
                    print(f"[DEBUG] Temp set to {current_temp:.1f}°C (instant)")
                else:
                    history = []
                    stable_count = 0
                    ...
                    while True:
                        ...
                        print(f"Temp stabilizing: target={current_temp:.1f}, actual={actual_temp:.2f}")
                        time.sleep(30)
                ...
                print(f"T={current_temp:.1f}°C, avg={avg:.3f}, min={min_val}, max={max_val}")

                current_temp += temp_step
                time.sleep(1)

            print("\n===== HIGH/LOW TEMP TEST 结果 =====")
            print("Temp, RawMean, RawMin, RawMax")
            for i in range(len(temp_data)):
                print(f"{temp_data[i]:.2f}, {adc_mean[i]:.3f}, {adc_min[i]}, {adc_max[i]}")
    ...
    def gpadc_temp_consistency_test(
        ...
        print(f"Running TEMP CONSISTENCY TEST with I2C address: 0x{device_addr:x}, Register: 0x{reg_addr:x}")
        ...
        while current_temp <= temp_max + 0.001:
            if stop_check and stop_check():
                print("Temp consistency test stopped by user.")
                break
            ...
            print(f"T={current_temp:.1f}°C  voltage sweep done ({len(mean_row)} points)")
            current_temp = round(current_temp + temp_step, 6)

        print("\n===== TEMP CONSISTENCY TEST 结果 =====")
        print("Temp\\Voltage  " + "  ".join(f"{v:.3f}" for v in voltage_points))
    ...
    def _calculate_gpadc_parameters(self, force_voltage_result):
        ...
        print(f"INL={params['inl']:.3f}, DNL={params['dnl']:.3f}, Gain Error: {params['gain_error']:.3f}%, Offset Error: {params['offset_error']:.3f} LSB")
        return params
    ...
    def _plot_voltage_adc_curve(self, voltage_data, mean_cali, adc_min_cali=None, adc_max_cali=None):
        try:
            ...
            try:
                ...
                print(f"Chart snapshot failed: {ex}")
                self._chart_image_bytes = None

        except Exception as e:
            print(f"Error plotting voltage-ADC curve: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _plot_temp_consistency_curves(self, result):
        try:
            ...
            print(f"Error plotting temp consistency curves: {e}")
            import traceback
            traceback.print_exc()
...
"""
            ...
            print(f"Error plotting temperature-ADC curve: {e}")
            import traceback
            traceback.print_exc()
