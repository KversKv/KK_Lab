...
class RegulationVoltageTestUI(N6705CConnectionMixin, QWidget):
    ...
    def __init__(self, n6705c_top=None, instrument_manager=None, ui_action_registry=None):
        ...
        self._config_memory = ConfigMemory("charger_test/regulation_voltage", self)
    ...
    def _register_ai_ui_actions(self):
        ...
        registry.register_many([
            UIActionSpec(
                id="charger_regulation_voltage.export_csv",
                label="导出结果 CSV",
                page_key="charger_regulation_voltage",
                handler=_wrap("导出结果 CSV", self._on_export_csv),
                risk="low",
                confirm=False,
                enabled_when=lambda: bool(self._export_data),
                description="把当前调压点测试结果导出为 CSV。需已有测试结果数据。",
            ),
        ])
