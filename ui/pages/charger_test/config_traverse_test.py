...
class ConfigTraverseTestUI(N6705CConnectionMixin, QWidget):
    ...
    def __init__(self, n6705c_top=None, instrument_manager=None, ui_action_registry=None):
        ...
        self._config_memory = ConfigMemory("charger_test/config_traverse", self)
    ...
    def _register_ai_ui_actions(self):
        ...
        registry.register_many([
            UIActionSpec(
                id="charger_config_traverse.export_csv",
                label="导出结果 CSV",
                page_key="charger_config_traverse",
                handler=_wrap("导出结果 CSV", self._on_export_csv),
                risk="low",
                confirm=False,
                enabled_when=lambda: bool(self._export_data),
                description="把当前配置遍历测试结果导出为 CSV。需已有测试结果数据。",
            ),
        ])
    ...
    def _build_config_card(self):
        ...
        self.reg_addr_edit = HexWheelLineEdit("0x0009")

        lbl_msb = QLabel("MSB")
        lbl_msb.setObjectName("fieldLabel")
        self.msb_edit = WheelLineEdit("9", wheel_step=1)

        lbl_lsb = QLabel("LSB")
        lbl_lsb.setObjectName("fieldLabel")
        self.lsb_edit = WheelLineEdit("5", wheel_step=1)
    ...
    #
    # Charger 配置遍历测试接入 AI 受控契约，薄封装既有方法：
    ...
    def ai_get_config(self) -> dict[str, Any] | None:
        try:
            ...
            _logger.error("AI 读取配置遍历测试配置失败", exc_info=True)
            return None
    ...
    def ai_start_test(self) -> tuple[bool, str]:
        ...
        self.append_log(
            f"[AI] 请求启动配置遍历测试：模式 {cfg.get('test_mode', 'Voltage')}，"
            f"代码范围 0x{cfg.get('min_code', 0):X}~0x{cfg.get('max_code', 0):X}。"
        )
        try:
            self._on_start_test()
        except Exception:  # noqa: BLE001 - 启动异常转可读结果
            _logger.error("AI 启动配置遍历测试失败", exc_info=True)
            return False, "启动测试异常，请查看日志。"
        if self.is_test_running:
            return True, "已请求启动配置遍历测试。"
    ...
    def ai_stop_test(self) -> tuple[bool, str]:
        ...
        try:
            ...
            _logger.error("AI 停止配置遍历测试失败", exc_info=True)
    ...
    def apply_config_to_controls(self, cfg: dict, silent: bool = False) -> tuple[bool, str]:
        ...
        # vmeter_channel / test_mode 是文本；iic_width 是 data；其余为 16 进制文本
        _set_combo_text(self.vmeter_channel_combo, "vmeter_channel")
        _set_combo_text(self.test_mode_combo, "test_mode")
        # 地址/代码以 16 进制文本写入（与 get_test_config 的 int 解析对应）
        if cfg.get("device_addr") is not None:
            self.device_addr_edit.setText(_to_hex(cfg["device_addr"]))
            applied.append("device_addr")
            touched.append(self.device_addr_edit)
        if cfg.get("reg_addr") is not None:
            self.reg_addr_edit.setText(_to_hex(cfg["reg_addr"]))
            applied.append("reg_addr")
            touched.append(self.reg_addr_edit)
        if cfg.get("msb") is not None:
            self.msb_edit.setText(str(cfg["msb"]))
            applied.append("msb")
            touched.append(self.msb_edit)
        if cfg.get("lsb") is not None:
            self.lsb_edit.setText(str(cfg["lsb"]))
            applied.append("lsb")
            touched.append(self.lsb_edit)
        if cfg.get("min_code") is not None:
            self.min_code_edit.setText(_to_hex(cfg["min_code"]))
            applied.append("min_code")
            touched.append(self.min_code_edit)
        if cfg.get("max_code") is not None:
            self.max_code_edit.setText(_to_hex(cfg["max_code"]))
            applied.append("max_code")
            touched.append(self.max_code_edit)
...
def _to_hex(val) -> str:
    """把 int/str 形式的地址/代码格式化为 16 进制字符串（配置回填辅助）。"""
    try:
