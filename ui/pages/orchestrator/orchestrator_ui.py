"""Orchestrator 主页面 —— 三栏布局可视化自定义测试序列编辑器"""
...
from ui.pages.orchestrator.node_palette import NodePalette, CollapsibleSection
from ui.pages.orchestrator.sequence_canvas import SequenceCanvas
from ui.pages.orchestrator.sequence_io import load_sequence_file
from ui.pages.orchestrator.property_panel import PropertyPanel
from core.orchestrator.nodes.base import BaseNode, get_node_class
from ui.pages.orchestrator.node_metadata import filter_selectable_ops, is_node_selectable
from core.orchestrator.context import ExecutionContext, StopExecution
from core.orchestrator.executor import ExecutorThread
from core.orchestrator.history import RunHistoryWriter
from core.orchestrator.paths import (
    get_primary_template_dir,
    record_recent_sequence,
    resolve_template_path,
)
from core.orchestrator.resolver import InstrumentResolver, collect_required_instrument_keys
...
from ui.pages.orchestrator.validation_panel import ValidationIssuesDialog
...
_PAGE_SVGS_DIR = os.path.join(
    get_resource_base(),
    "resources", "pages", "orchestrator_SVGs"
)
...
class OrchestratorUI(N6705CConnectionMixin, ChamberConnectionMixin, SerialComMixin, QWidget):
    """Orchestrator 主界面"""
    ...
    # 测试序列执行完成（success, message）：供 AI 异步动作回灌续跑（§4 / S3-2）
    ...
    def _build_ui(self) -> None:
        ...
        self.page_title = QLabel("Orchestrator")
    ...
    def get_ai_test_config(self) -> Optional[Dict[str, Any]]:
        """AI get_current_test_config 动作的快照源：返回当前 Orchestrator 配置。
        ...
        return {
            ...
            "sequence": sequence,
            "_message": f"当前 Orchestrator 配置：{len(nodes)} 个顶层节点。",
        }
    ...
    def ai_run_single_step(self, step_id: str) -> tuple[bool, str]:
        ...
        try:
            self._context.acquire_leases(owner="orchestrator")
    ...
    def _find_node_by_uid(nodes: List[BaseNode], uid: str) -> Optional[BaseNode]:
        ...
        for node in nodes:
            if node.uid == uid:
                return node
            for child in getattr(node, "children", []) or []:
                found = OrchestratorUI._find_node_by_uid([child], uid)
    ...
    def _on_add_btn_clicked(self) -> None:
        ...
        from core.orchestrator.nodes.base import get_nodes_by_category
        from ui.pages.orchestrator.node_palette import INSTRUMENT_REGISTRY
    ...
    def _on_instrument_requested(self, instr_id: str) -> None:
        ...
        from ui.pages.orchestrator.node_palette import get_instrument_by_id
    ...
    def _on_prompt_requested(self, request: _PromptRequest) -> None:
        ...
        box.setWindowTitle("Orchestrator Prompt")
    ...
    def _on_execution_finished(self, success: bool, message: str) -> None:
        ...
        # 通知 AI 异步动作层（§4 / S3-2）：序列执行结束，触发续跑回灌
        record_count = 0
        if self._context is not None:
            try:
                record_count = len(self._context.records or [])
            except Exception:  # noqa: BLE001
                record_count = 0
        summary = message or ("PASS" if success else "FAIL")
        self.sequence_execution_finished.emit(
            bool(success), f"{summary}（记录 {record_count} 行）"
        )
    ...
    def cleanup_threads(self) -> None:
        """清理工作线程"""
        if self._active_prompt_request is not None:
            self._active_prompt_request.cancel("Orchestrator 页面关闭")
        if self._active_prompt_box is not None:
            self._active_prompt_box.reject()
