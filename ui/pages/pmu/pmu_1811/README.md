...
- **LDO** — 寄存器模型（9 个关键位域 `pu` / `pu_dr` / `pu_status` / `lp` / `lp_dr` / `vbit_*` / `res_sel_dr`）
  ...
  - BUCK 与 LDO 的差异
- **DCDC** — 规划中
- **BUCK** — 寄存器模型（11 个位域, 无 `lp` / `lp_dr`, 有 `res_sel_dr` + 4 个 `bg_en` / `sw_en` 系列）+ 使能 + 电压已补全（5 步 bg_en/sw_en 调压序列）; 模式切换待补全
...
4. **并联对偶短接线**（紫色半透明竖线，`_draw_pairs`）— BUCK↔LDO 输出端短接
   两卡片同列（`CARD_X_L1`），卡片右边（`CARD_X_L1 + CARD_W`）用竖线连接，
   表示两路输出并联到同一轨
5. **子母线药丸**（深灰底 + 蓝字）
