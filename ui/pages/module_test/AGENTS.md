...
- **ARB 残留清理**：`clear_arb_all_channels` 的 `ABOR:TRAN` 被裸 except 吞掉且未等 initiated 清零就写 `VOLT/CURR:MODE FIX`（连续脉冲时清除慢，此时写 FIX 报 +308 被忽略），Load Transient 的 `CURR:MODE ARB` 会残留到 Line Transient。transient 项开局改走 `_common._reset_arb_state`：先 `arb_stop` + 逐通道 `wait_arb_idle`，再显式逐通道 `exit_arb_voltage` + `exit_arb_current`，确保通道回到固定输出态。
...
- **示波器通道显示联动**：`run_load_capability_ripple` 开局显式 `set_channel_display(scope_ch, True)`——上一项 transient 调过 `close_all_channels()`，不显式开则 ripple 全程平线；`dcdc_switching_freq` 补 SW 通道 display + 按 `fsw_expected_khz`（新项级参数）设时基；`_measure_with_autoscale` 重试耗尽抛异常时外层先 `ctx.scope.run()` 恢复采集再降级，避免示波器停在 stop 态污染下一组/项。
