...
- **Transient 截图 settle 下限（2026-07 真机调试）**：`run_load_transient` 改时基/通道后先 `ctx.scope.run()` 再 `settle(max(2.5, 6×(period/2)))`，然后 `_measure_with_autoscale`（内部 stop+测量）返回时直接截图，**不再多余 run/stop**。两个坑：①改时基前必须确保示波器在 run 态——停采态下改时基只是把旧波形重绘，settle 再久也采不到新波形（截图定格在过渡帧）；②settle 下限 2.5s 不可省——低频组（10Hz 时 6×半周期仅 0.3s）DSOX 在新时基下采满一屏+刷新显示较慢，短 settle 会让 autoscale 的 stop 定格在未采满的空帧/旧帧上（调试 settle 截图的 ~2.2s 传输延迟曾"碰巧"掩盖此问题，去掉调试代码即复现）。
