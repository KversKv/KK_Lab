...
from PySide6.QtWidgets import (
    ...
    QSizePolicy, QButtonGroup, QFileDialog
)
...
class DCDCEfficiencyTestThread(QThread):
    ...
    def run(self):
        try:
            cfg = self._cfg
            start_a = cfg["start_current_a"]
            end_a = cfg["end_current_a"]
            ...
            if sweep_mode == "Log":
                ...
                total_decs = abs(log_end - log_start)
                total_points = max(2, int(round(total_decs * points_per_dec)) + 1)
                step = (log_end - log_start) / (total_points - 1)
                current_points = [10 ** (log_start + i * step) for i in range(total_points)]
            else:
                step_a = cfg["step_current_a"]
            ...
            sleep_settle = 0.0 if self._debug else 0.2
            sleep_measure = 0.0 if self._debug else 0.3
            ...
            self.log_message.emit(f"[TEST] Baseline Vin={vin_base:.3f}V  Vout={vout_base:.3f}V  Iin={iin_base:.6f}A")
            ...
            for i_set in current_points:
                if self._stop_flag:
                    self.log_message.emit("[TEST] Stopped by user.")
                    break

                n.set_current(iload_ch, i_set)
                ...
                eff = (vout * max(i_out, 1e-9)) / denom
                eff = max(min(eff, 1.2), 0.0)
                eff_pct = eff * 100

                self.log_message.emit(
                    f"[MEAS] Iset={i_set*1000:.1f}mA  Vin={vbat:.3f}V  "
                    f"Vout={vout:.3f}V  Iin={i_in:.5f}A  Iout={i_out:.5f}A  "
                    f"Eff={eff_pct:.2f}%"
                )

                output.append((abs(i_out), eff_pct))
                sum_eff += eff_pct
                if eff_pct > max_eff:
                    max_eff = eff_pct

                self.chart_point.emit(abs(i_out), eff_pct)
                ...
                avg_eff = sum_eff / len(output)
                result = {
                    "efficiency": avg_eff,
                    "max_efficiency": max_eff,
                    "total_power": vbat * i_in,
                }
                self.result_update.emit(result)

            if output:
                n.set_current(iload_ch, 0.0)
                n.channel_off(iload_ch)
...
class PMUDCDCEfficiencyUI(QWidget):
    ...
    def _create_layout(self):
        ...
        self.efficiency_voltage_card = self._create_mini_stat("最大效率电压", "---")
        self.power_card = self._create_mini_stat("总功耗", "---")

        stat_layout.addWidget(self.efficiency_card["frame"])
        stat_layout.addWidget(self.max_efficiency_card["frame"])
        stat_layout.addWidget(self.efficiency_voltage_card["frame"])
        stat_layout.addWidget(self.power_card["frame"])
    ...
    def _build_channel_card(self):
        ...
        self.vin_channel_combo = QComboBox()
        ...
        self.vout_channel_combo = QComboBox()
        ...
        self.cc_load_channel_combo = QComboBox()
    ...
    def _build_sweep_card(self):
        ...
        self.log_mode_btn.setChecked(True)
        ...
        self.load_current_start_spin.setRange(0.001, 100.0)
        self.load_current_start_spin.setDecimals(3)
        self.load_current_start_spin.setSingleStep(0.01)
        self.load_current_start_spin.setValue(0.01)

        self.lbl_end = QLabel("End Current (A)")
        self.lbl_end.setObjectName("fieldLabel")
        self.load_current_end_spin = QDoubleSpinBox()
        self.load_current_end_spin.setRange(0.001, 100.0)
        self.load_current_end_spin.setDecimals(3)
        self.load_current_end_spin.setSingleStep(0.1)
        self.load_current_end_spin.setValue(2.0)

        self.lbl_step = QLabel("Step Current (A)")
        self.lbl_step.setObjectName("fieldLabel")
        self.step_current_spin = QDoubleSpinBox()
        self.step_current_spin.setRange(0.001, 100.0)
        self.step_current_spin.setDecimals(3)
        self.step_current_spin.setSingleStep(0.01)
        self.step_current_spin.setValue(0.1)
    ...
    def _update_chart_point(self, i_out_a, eff_pct):
        if HAS_QTCHARTS and hasattr(self, 'series'):
            self.series.append(i_out_a * 1000, eff_pct)

            pts = self.series.points()
            if pts and self.axis_x is not None:
                min_x = min(p.x() for p in pts)
                max_x = max(p.x() for p in pts)
                is_log = isinstance(self.axis_x, QLogValueAxis)
                if is_log:
                    if min_x > 0 and max_x > 0 and min_x != max_x:
                        self.axis_x.setRange(min_x * 0.8, max_x * 1.2)
                else:
                    margin = max((max_x - min_x) * 0.05, 1.0)
                    self.axis_x.setRange(max(0, min_x - margin), max_x + margin)

            if pts and self.axis_y is not None:
                min_y = max(0, min(p.y() for p in pts) - 5)
                max_y = min(100, max(p.y() for p in pts) + 5)
    ...
    def update_test_result(self, result):
        ...
        if "max_efficiency_voltage" in result:
            self.efficiency_voltage_card["value"].setText(f"{result['max_efficiency_voltage']:.2f} V")
        if "total_power" in result:
            self.power_card["value"].setText(f"{result['total_power']:.4f} W")

        log_msg = "[MEASURE] "
        msg_parts = []
        if "efficiency" in result:
            msg_parts.append(f"Efficiency: {result['efficiency']:.2f}%")
        if "max_efficiency" in result:
            msg_parts.append(f"Max: {result['max_efficiency']:.2f}%")
        if "total_power" in result:
            msg_parts.append(f"Power: {result['total_power']:.4f} W")
        if msg_parts:
    ...
    def clear_results(self):
        ...
        self.efficiency_voltage_card["value"].setText("---")
