...
class PMUThresholdUI(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for cb in (self.is_gain_enable_checkbox, self.threshold_enable_checkbox):
            if cb.isVisible() and cb.isChecked():
                opt_rect = cb.rect()
                indicator_x = 0
                indicator_y = (opt_rect.height() - 15) // 2
                x1 = cb.mapTo(self, opt_rect.topLeft()).x() + indicator_x + 3
                y1 = cb.mapTo(self, opt_rect.topLeft()).y() + indicator_y + 8
                x2 = cb.mapTo(self, opt_rect.topLeft()).x() + indicator_x + 6
                y2 = cb.mapTo(self, opt_rect.topLeft()).y() + indicator_y + 11
                x3 = cb.mapTo(self, opt_rect.topLeft()).x() + indicator_x + 12
                y3 = cb.mapTo(self, opt_rect.topLeft()).y() + indicator_y + 4
