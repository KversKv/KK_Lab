...
- **SerialCom 多开串口记忆**：持久化 JSON 含 `extra_panels`（内嵌面板 config）与 `independent_windows`（浮窗 config+`geometry`）；重开时 `_sc_apply_persisted_state` 尾部调 `_sc_restore_persisted_panels()` 恢复布局/几何，**只恢复布局不自动回连**（历史 `connected` 键读取后丢弃，`auto_connect` 强制置 False，恢复后面板/浮窗均保持未连接）。改面板 config 结构须同步采集（`_sc_collect_persisted_state`）与恢复（`log_panel_mixin`）两端。
