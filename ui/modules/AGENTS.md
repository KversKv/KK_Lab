...
- **SerialCom 多开串口记忆**：持久化 JSON 含 `extra_panels`（内嵌面板 config+`connected`）与 `independent_windows`（浮窗 config+`connected`+`geometry`）；重开时 `_sc_apply_persisted_state` 尾部调 `_sc_restore_persisted_panels()` 恢复，`connected` 映射为 `auto_connect`。改面板 config 结构须同步采集（`_sc_collect_persisted_state`）与恢复（`log_panel_mixin`）两端。
