...
- **SerialCom 多面板控制跟随**：顶部 Connect/Pause/Stop 按 `_sc_active_log_panel_index` 分发到聚焦面板（Refresh 保持全局端口扫描）；焦点切换 / 面板连接变化统一经 `_sc_sync_top_control_state()` 回同步按钮文本/图标/勾选，新增面板控制入口必须调用它。额外面板 `paused=True` 时在 `_sc_extra_panel_on_data` 入口丢数据（与主面板语义一致）。**未连接时 Pause/Stop 强制 `setEnabled(False)`**，使能唯一由 `_sc_sync_top_control_state()` 按聚焦面板连接态驱动（启动 / 连接 / 断开 / 焦点切换均须过此方法）。**Pause/Stop 互斥（后点生效）**：`_sc_on_pause(True)` 释放 `stopped`、`_sc_on_stop(True)` 释放 `paused`+`set_display_paused(False)`（主/额外面板同一语义），禁止两者同时勾选。**侧栏 Port 使能唯一由 `_sc_sync_sidebar_to_focus()` 驱动**：焦点未变早退分支也须按聚焦面板连接态刷新 `setEnabled`（同面板 Connect/Disconnect 场景），禁止绕过它直控。**主面板改名**：标题真值 = `_sc_primary_panel_title`（`ui.primary_panel_title` 持久化，采集/恢复/重置三端同步），右键 Settings 走 `_PanelSettingsDialog(name_only=True)` 仅改名，串口参数入口仍在侧栏不重复。
- **SerialCom 多开串口记忆**：持久化 JSON 含 `extra_panels`（内嵌面板 config+`connected`）与 `independent_windows`（浮窗 config+`connected`+`geometry`）；重开时 `_sc_apply_persisted_state` 尾部调 `_sc_restore_persisted_panels()` 恢复，`connected` 映射为待回连登记。**连接恢复 = 非阻塞单次尝试**：恢复时只建面板并登记 `_sc_pending_autoconnect_panels/_windows`（浮窗 `auto_connect` 强制置 False 防自行连接），首次端口枚举完成（热插拔基线广播 `_sc_on_hotplug_ports_changed` 入口，或 800ms receiver-timer 兜底）后 `_sc_try_pending_autoconnect()` 统一试连一次，失败仅打日志不重试；清单消费后即空，运行期热插拔不再触发回连（禁止）。改面板 config 结构须同步采集（`_sc_collect_persisted_state`）与恢复（`log_panel_mixin`）两端。
...
- **SerialCom 三处容器均带 Save 按钮，保存状态各自独立**：主面板 = Mixin 单份 `_sc_save_handle/_path`（`filter_save_mixin._sc_on_save_toggle`）；额外面板 = dict 内 `save_handle/save_path/save_keep_timestamp`（`_sc_extra_panel_on_save_toggle/_start_save/_stop_save/_write_save`）；独立浮窗 = `_IndependentSerialWindow` 自管（`_on_save_toggle/_start_save/_stop_save/_write_save_line`）。写入统一走组件 `raw_appended` 钩子；移除面板（`_sc_on_remove_log_panel`/`_sc_remove_specific_panel`）、`close_serial()`、浮窗 `closeEvent` 必须先 stop save 再清理。
