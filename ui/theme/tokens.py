...
# ---------------------------------------------------------------------------
# Module Test 页面 token（2026-08 视觉重构引入；2026-09 主题适配统一）。
# 历史上是独立 GitHub-dark 风色板（灰蓝底 + 蓝主色），与其它页面观感割裂；
# 现与全局 dark_tokens() 同盘（深蓝底 + 紫主色），仅保留页面专属几何差异。
# 新增语义 token（surface_hover/surface_selected/text_data/progress_track 等）
# 仍由 qss/module_dark.qss 经 apply_qss(**overrides) 注入，不在此扩展 Tokens 字段。
def module_dark_tokens() -> Tokens:
    """Module Test 页面 token：全局 dark 调色板 + 本页几何差异（行高 34）。

    保留独立函数名仅为稳定既有调用点（badge_delegate / test_plan_panel /
    module_theme）；色板与 ``dark_tokens()`` 完全一致。
    """
    return replace(dark_tokens(), name="module_dark", table_row_h=34)
