...
任务"怎么调用 / 怎么执行 / 怎么回归"走统一 SOP：
🔴 [docs/ai/09_WORKFLOW.md](./docs/ai/09_WORKFLOW.md)

用户说 **"按 SOP 做"** / **"走 09_WORKFLOW"** 即触发完整三阶段流程（调用期 → 执行期 → 回归期）。

---

## 7. 任务 → docs/ai 分发索引（CRITICAL · AI 必读）

> 🔴 **强制要求**：开始任务前，先在下表定位任务类型，**必须通读对应"必读"文档**再动手；"可选"文档按需拉取。
> 本索引同时镜像在 [AGENTS.md §分发索引](./AGENTS.md#分发索引)。
