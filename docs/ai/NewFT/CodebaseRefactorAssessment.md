...
>
> **2026-06-24 视图瘦身（次项·consumption_test 拆视图）**：新建 [ui/pages/consumption_test/view_panels.py](../../../ui/pages/consumption_test/view_panels.py)（`ConsumptionTestViewPanelsMixin`），平移 8 个纯视图构建方法（`_create_layout` / `_create_connection_panel` / `_create_firmware_and_config_panels` / `_build_firmware_serial_widgets` / `_create_consumption_test_panel` / `_create_test_buttons_row` / `_setup_bin_result_table` / `_add_bin_result_row`），加入 `ConsumptionTestUI` 继承链（MRO 验证通过）。**主壳 3393 → 2558 行**（-835）。`consumption_test/` 现拆为：主壳 2558 + view_config 651 + view_results 125 + view_panels ~890 + widgets 379。回归：offscreen 实例化全 UI 构建链跑通、consumption_controller 21/21、smoke import（含 compile_all）全 PASS，零行为漂移、零编码损坏。
>
> **方法论沉淀**：Python 脚本批量提取/删除方法体时，外迁 Mixin 需独立补全运行期依赖 import（本项补 `SUPPORTED_CHIPS` / `SCROLLBAR_STYLE` / `QColor`）——静态编译通过 ≠ 运行通过，必须做 offscreen 实例化验证。
