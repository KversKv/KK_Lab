...
- **涉及文件**:
  ...
  - `ui/pages/custom_test/node_palette.py`
  - `presets/custom_test_templates/*.json`
- **预计工作量**: 0.5 会话
- **预计会话**: 会话 1
- **完成日期**: 2026-05-28
...
- **预计会话**: 会话 16
- **完成日期**: 2026-05-29
- **完成内容**: 新增 4 个 v2 样例模板：`sample_n6705c_voltage_sweep.json`、`sample_i2c_register_sweep.json`、`sample_chamber_n6705c_loop.json`、`sample_uart_send_receive.json`，每个模板均带 `metadata.required_capabilities`；模板目录后续应统一为 `presets/custom_test_templates`，并由打包配置按该目录收集运行时数据。
