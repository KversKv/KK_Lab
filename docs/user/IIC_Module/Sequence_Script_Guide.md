...
- Script 描述「按什么顺序操作设备」。
- 脚本可通过 YAML 顶层 `template` 字段挂到一个模板名上，便于在 UI 列表中按模板过滤。
...
| 单文件命名 | `<safe_name>.yaml` | `safe_name` 由脚本 `name` 经 `re.sub(r'[^\w\-.]', '_', name)` 生成 |
...
> 模板与脚本目录相互独立，可同名。脚本文件扩展名必须为 `.yaml` 或 `.yml`。
...
4. 修改 Name / Desc / Tpl。
5. 点 **Save** → 写入 `i2c_sequences/<name>.yaml`，列表自动刷新并选中。
...
- Table → YAML：切到 YAML 模式或点 Save 时，把表格内容序列化为 YAML。
- YAML → Table：切回 Table 模式时解析 YAML 刷新表格。
- Name / Desc / Tpl 三个输入框与 YAML 顶层字段双向绑定，任一处修改都会同步。
...
```
script       := yaml_document
yaml_document := { name, description, template, commands: [line*] }
...
DSL 不支持算术，但可以通过外部脚本生成 commands 列表后塞进 YAML：
...
with open("user_data/i2c_sequences/eeprom_full_dump.yaml", "w", encoding="utf-8") as f:
    yaml.dump(out, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
```

生成后直接放入 `user_data/i2c_sequences/` 即可被 UI 列表识别。
