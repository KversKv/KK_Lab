...
> **集合与模板的对应约定**：默认按模板名分文件，例如模板 `SY6105_Charger` 的所有脚本放在 `SY6105_Charger.yaml` 内。这并非强制——一个集合可含不同 `template` 的脚本，但按模板分文件最利于维护。
...
├── SY6105_Charger.yaml          # 集合：SY6105 充电器全部脚本
...
4. 修改 Name / Desc / Tpl。
5. 点 **Save**：
   - 新建脚本无来源集合时，默认写入与 `template` 同名的集合文件（如 `SY6105_Charger.yaml`）。
...
- [ ] 集合文件名建议与主模板名一致（如 `SY6105_Charger.yaml`），便于 UI 过滤。
